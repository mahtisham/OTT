...
class HomeController extends Controller
{
    ...
    public function index(Request $request, $menu_slug)
    {
        ...
        elseif ($this->configs->prime_genre_slider == 1) {
            //Layout 1
            //return $usercountry;
            return view('home', compact('menu_data', 'recent_data', 'home_slides', 'ad', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed', 'watchistory', 'audiolanguages',  'getallgenre', 'blogs', 'liveevent', 'protip', 'audio', 'short_promo', 'audio_lang', 'upcomingitems', 'getallaudiolanguage', 'top_data'));
        } else {
            //Layout 2
            return view('home2', compact('menu_data', 'recent_data', 'home_slides', 'ad', 'age', 'genres', 'menuh', 'catlog', 'withlogin', 'menu', 'subscribed', 'watchistory', 'audiolanguages',  'getallgenre', 'blogs', 'liveevent', 'protip', 'audio', 'short_promo', 'audio_lang', 'upcomingitems', 'getallaudiolanguage', 'top_data'));
        }
    ...
    public function showallgenre(Request $request, $id)
    {
        ...
        if (isset($genre)) {
            ...
            $tvs = TvSeries::where('genre_id',  $genre->id)->where('status', 1)->whereHas('seasons_first')->get();
    ...
    public function showallalang(Request $request, $id)
    {
        ...
        if (isset($alang)) {
            $items = collect();
            $movies = Movie::where('a_language', $alang->id)->where('status', 1)->get();

            foreach ($movies as $movie) {
                $items->push($movie);
            }

            $tvs = Season::where('a_language', $alang->id)->get();
